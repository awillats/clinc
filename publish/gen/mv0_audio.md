...
Note that (W to the T) s sub j gives the variance at node j due to
...
[^23]: not 100% sure this is true, the empirical results are really
    ...
    be something like d|R| over dS or dr squared over dS since these
    effects decrease the *magnitude* of correlations. I.e. if
    d|R| over dS < 0 increasing S might move r from -0.8 to
